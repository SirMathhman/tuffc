...
const PANIC_CONVERSIONS = [
  ...
  {
    match: /panic\("'expect'\/'actual' are not supported on extern declarations"\)/g,
  ...
  {
    match: /panic\("'extern' modifier must be followed by fn, let, or type declaration"\)/g,
  ...
  {
    match: /panic\("'expect'\/'actual' are currently supported only on fn declarations"\)/g,
  ...
  {
    match: /panic\("Only 0 or 0USize is supported as a type-level numeric sentinel"\)/g,
