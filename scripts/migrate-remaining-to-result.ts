...
`;
  ...
  return content.slice(0, firstLetMatch.index) + imports + content.slice(firstLetMatch.index);
}
...
function migrateFile(filePath: string): boolean {
  ...
  let modified = false;
  const errorType = FILE_ERROR_TYPES[path.relative(TUFFC_ROOT, filePath).replace(/\\/g, '/')];
  ...
  console.log(`  ℹ️  Remaining work: Convert panic_with_code* calls to helper functions`);
...
function main() {
  ...
  const files = Object.keys(FILE_ERROR_TYPES).map(f => path.join(TUFFC_ROOT, f));
  ...
  console.log(`✅ Pattern proven in Phases 1-3 (resolver, typechecker, parser)`);
