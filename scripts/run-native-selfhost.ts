...
if (!fs.existsSync(exePath)) {
  console.error(
    `[native-selfhost] Missing native compiler executable: ${exePath}\n` +
      "Run `npm run native:selfhost:parity` first to generate it.",
  );
  process.exit(1);
}
...
if (run.error) {
  if ((run.error as NodeJS.ErrnoException).code === "ETIMEDOUT") {
    console.error(
      `[native-selfhost] TIMEOUT: process exceeded ${TIMEOUT_MS / 1000}s — killed`,
    );
    process.exit(124);
  }
  console.error(`[native-selfhost] ERROR: ${run.error.message}`);
  process.exit(1);
}
