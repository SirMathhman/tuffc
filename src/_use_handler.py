...
def try_handle_use_statement(s: str, env: dict) -> str | None:
    ...
    m = re.match(r"^\s*use\s+([A-Za-z_]\w*)::\s*([A-Za-z_]\w*)\s*;?\s*(.*)", s, re.DOTALL)
    if not m:
