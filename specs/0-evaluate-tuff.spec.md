...
- Basic arithmetic expressions using `+`, `-`, `*`, `/`, and `%` should be supported, including parentheses and standard precedence.
- `stdIn` tokens should be consumed from left to right for multiple `read<T>()` calls.
...
- Division by zero and modulo by zero should throw `RangeError`.
...
- As a developer, I want basic arithmetic operators and precedence rules so that I can write ordinary numeric expressions.
...
- As a developer, I want invalid math such as division by zero to fail with `RangeError` so that bad expressions are rejected.
